...
::: {.video-container}
{{< video static/videos/working_lane_detection.mov >}}
:::
...
After much testing, we were able to create a proper lane following algorithm that is ready for the Highbay car. 
Since the simulation had 2 lanes, the code struggled a bit after a turn identifying which lane was the right one. However, this will not be an issue on the Highbay as there is just one lane.
:::
:::
...
::: {.result-card}
![Lane Detection in Highbay track](static/images/highbay_lane.png){.highbay-lane-img}
:::

::: {.content-text}
We were able to get the car to detect the lanes on the real-life track properly. One thing that we found was that when it was too funny, we ran into an issue of having too much noise. As a result, our bev image had much white.
:::
:::
...
::: {.result-card}
![Obstacle Detection in simulation](static/images/highbay_lane.png){.highbay-lane-img}
:::

::: {.content-text}
Using a state machine, we were able to get the car to detect an obstacle such as a stop sign. 
Once it detects the sign, it will stop for 2 seconds, and then keep going. So, our car is functining as we wanted it to.
:::
:::
